...
def get_collection(db):
    ...
    if "cars" not in db.list_collection_names():
        df = pd.read_csv(
            "https://raw.githubusercontent.com/iantonios/dsc205/refs/heads/main/cars.csv"
        )
...
def get_user_selections():
    with st.form(key="my_form"):
        ...
        min_mpg = st.slider("Minimum fuel efficiency (highway - mpg)", 0, 55, 20)
        car_type = st.radio("Car type", ("Sedan", "Wagon", "SUV", "Sports Car"))
        # New text input field
        car_make_input = st.text_input("Enter a car make")
        submitted = st.form_submit_button(label="Process")

        if submitted:
            return retail_range, min_mpg, car_type, car_make_input
        else:
            return ((20000, 50000), 20, "Sedan", "Acura")


def exec_query(collection, retail_range, min_mpg, car_type, car_make):
...
def main():
    ...
    st.subheader("Activity 10: Asante Frye")  # Added sub-header with name
    db = connect()
    collection = get_collection(db)
    retail_range, min_mpg, car_type, car_make = get_user_selections()
    # Include new make input in query
    results = exec_query(collection, retail_range, min_mpg, car_type, car_make)
    st.markdown("---")
